...
float Txbox = 0.5, currentX = 0.0, previousX = 0.0;
...
void balance() {// put your main code here, to run repeatedly:
  ...
  if ((currentT - previousT)/1000.0 >= Ts) {
    ...
    if (abs(roll_angle) < 20 && start && !reset){
      digitalWrite(BRAKE1, HIGH);
      digitalWrite(BRAKE2, HIGH);

      theta += theta_dot * Ts;
      psi_dot = NIDEC1_ENC.getCount()*pulse2rad/Ts - psidot_ref;
      psi += psi_dot*Ts;
